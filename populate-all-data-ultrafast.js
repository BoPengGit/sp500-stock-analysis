...
const CALLS_PER_MINUTE = 700; // 700 calls/min - safe under 750 limit
...
async function fetchAllDataForYear(symbol, year, quarter) {
  const data = {};

  try {
    // Fetch key metrics sequentially to respect rate limit
    const keyMetricsData = await rateLimitedFetch(`https://financialmodelingprep.com/api/v3/key-metrics/${symbol}?period=quarter&limit=80&apikey=${FMP_API_KEY}`);
    const incomeData = await rateLimitedFetch(`https://financialmodelingprep.com/api/v3/income-statement/${symbol}?period=quarter&limit=80&apikey=${FMP_API_KEY}`);
    const cashFlowData = await rateLimitedFetch(`https://financialmodelingprep.com/api/v3/cash-flow-statement/${symbol}?period=quarter&limit=80&apikey=${FMP_API_KEY}`);
    const ratiosData = await rateLimitedFetch(`https://financialmodelingprep.com/api/v3/ratios/${symbol}?period=quarter&limit=80&apikey=${FMP_API_KEY}`);
...
async function populateAllMissing(db) {
  ...
  for (const record of records) {
    ...
    try {
      ...
      const fetchedData = await fetchAllDataForYear(symbol, year, quarter);

      // Calculate ADTV if missing
  ...
  console.log(`\nSummary: ${updated} updated, ${skipped} skipped (no new data)`);
}
...
async function main() {
  ...
  console.log('POPULATING ALL MISSING FUNDAMENTAL DATA - ULTRAFAST');
  console.log('='.repeat(60));
