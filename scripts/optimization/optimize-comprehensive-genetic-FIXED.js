 ...
 * FIXED: Now uses /hold-winners endpoint instead of /annual-rebalance
 */
async function testSinglePortfolio(weights, portfolioSize, threshold) {
  try {
    const params = {
      ...
      keepThreshold: threshold  // FIXED: Changed from sellThreshold to keepThreshold
    };

    // FIXED: Changed endpoint from /annual-rebalance to /hold-winners
    const response = await axios.get(`${API_BASE}/stocks/hold-winners`, {
      params,
      timeout: 30000
    });

    // FIXED: hold-winners returns data in different format
    const data = response.data.data?.results?.['5year'];
    if (!data) return null;

    // Return the total return directly
...
async function runGeneticAlgorithm() {
  ...
  for (let gen = 0; gen < GENERATIONS; gen++) {
    ...
    if (generationsWithoutImprovement >= 20) {
      console.log(`\n⏹️  Early stopping: No improvement for 20 iterations`);
      break;
    }
  ...
  globalBest.results.forEach((ret, i) => {
    ...
    console.log(`  ${config.size} stocks @ keep top ${config.threshold}: ${ret.toFixed(2)}%`);
  });
