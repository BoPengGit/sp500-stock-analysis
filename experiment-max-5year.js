...
/**
 * Experiment: Find Weight Configuration that Maximizes 5-Year Returns
 *
 * This script tests various weight combinations to find the configuration
 * that produces the best 5-year returns for the top 20 stocks portfolio.
 */
...
async function runExperiment() {
  console.log('=== 5-Year Returns Optimization Experiment ===\n');
  ...
  // Sort results by 5-year return (descending)
  results.sort((a, b) => b.returns.fiveYear - a.returns.fiveYear);

  // Display top 20 results
  console.log('=== TOP 20 CONFIGURATIONS BY 5-YEAR RETURN ===\n');
  console.log('Rank | Config (MC-ADTV-PS-SG-GF) | 5Y Return | 1Y    | 2Y    | 3Y    | 4Y    | Avg   | Valid (5Y)');
  console.log('-----|---------------------------|-----------|-------|-------|-------|-------|-------|------------');
  ...
  for (let i = 0; i < Math.min(20, results.length); i++) {
    ...
    console.log(
      ...
      `${r.returns.fiveYear.toFixed(2).padStart(8)}% | ` +
  ...
  if (results.length > 0) {
    ...
    console.log('\n=== OPTIMAL CONFIGURATION FOR 5-YEAR RETURNS ===');
    ...
    console.log(`\n5-Year Return: ${best.returns.fiveYear.toFixed(2)}% (${best.validStocks.fiveYear} stocks)`);
    ...
    for (const label of currentConfigs) {
      ...
      if (found) {
        ...
        console.log(`  ${label}: Rank #${rank}, 5Y: ${found.returns.fiveYear.toFixed(2)}%`);
      }
    }
  }
