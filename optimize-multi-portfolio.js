...
async function evaluateFitness(weights) {
  try {
    ...
    // Test each portfolio configuration
    ...
    // Calculate average return across all configurations
...
async function runGeneticAlgorithm() {
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║   MULTI-PORTFOLIO OPTIMIZATION - 10 METRIC WEIGHTS           ║');
  console.log('║   Optimizing for AVERAGE return across 4 configurations      ║');
  console.log('╚═══════════════════════════════════════════════════════════════╝\n');

  console.log('Portfolio Configurations:');
  PORTFOLIO_CONFIGS.forEach((config, i) => {
    console.log(`  ${i+1}. ${config.size} stocks, ${config.threshold}% sell threshold`);
  });
  console.log();
  ...
  for (let i = 0; i < POPULATION_SIZE; i++) {
    ...
    population.push({ weights, fitness: null, results: null });
  }
  ...
  for (let gen = 0; gen < GENERATIONS; gen++) {
    ...
    if ((!globalBest || genBest.fitness > globalBest.fitness) && genBest.fitness !== -Infinity && genBest.results !== null) {
      ...
      console.log(`   Individual Returns:`);
      genBest.results.forEach((ret, i) => {
        console.log(`     ${PORTFOLIO_CONFIGS[i].size} stocks @ ${PORTFOLIO_CONFIGS[i].threshold}% thresh: ${ret.toFixed(2)}%`);
      });
    } else {
    ...
    for (let i = 0; i < ELITE_SIZE; i++) {
      nextGen.push({ ...population[i], fitness: null, results: null });
    }
    ...
    while (nextGen.length < POPULATION_SIZE) {
      ...
      nextGen.push({ weights: childWeights, fitness: null, results: null });
    }
  ...
  console.log(`Average 5-Year Return: ${globalBest.fitness.toFixed(2)}%\n`);

  console.log('Returns by Configuration:');
  globalBest.results.forEach((ret, i) => {
    const config = PORTFOLIO_CONFIGS[i];
    console.log(`  ${config.size} stocks @ ${config.threshold}% threshold: ${ret.toFixed(2)}%`);
  });
